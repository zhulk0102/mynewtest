...
from config.settings import untilsPath
...
class ReportTestResult(unittest.TestResult):
    ...
    def stopTest(self, test) -> None:
        ...
        # e= d.split(".")[0]
        # print("{}".format(e))
        a = str(time.time() - self.start_time).split(".")[0]
        self.end_time = "{} s".format(a)
        ##########################################################
        # ###201812.24myh注释  修改用例执行时长切割字符串 表示小数部分输出的有效位数, 如果是字符串 精度就是输出的最大长度
    ...
    def complete_output(self,test):
        ...
        if self.sys_stdout:
            ...
            self.sys_stdout = None
            self.sys_stdout = None
        # item = self.outputBuffer.getvalue()
        # print(type(item))
        # print(type(test))
        # printt(test.__dict__['_testMethodName'])
        # self.writeItem(test._testMethodName)
        return self.getLog(test)  # self.outputBuffer.getvalue()

    def getLog(self,test):
        fileName = time.strftime('%Y-%m-%d.log')
        # fileName = test.__dict__['_testMethodName']+".log"
        # filePath = os.path.abspath(self.log_path) if \
        #     os.path.abspath(self.log_path).endswith('/') else \
        #     os.path.abspath(self.log_path) + '/'
        fillFullPath = os.path.join(self.log_path, fileName)
        if os.path.exists(fillFullPath) is False:
            return ""
        else:
            result = open(fillFullPath, 'rb').readlines()
            st = ""
            for i in result:
    ...
    def stopTestRun(self, title=None) -> dict:
        ...
        :return:
        """
        # self.writeItem(str(FIELDS))
        FIELDS['testName'] = title if title else self.default_report_name
        FIELDS['testAll'] = len(self.result_list) + int(FIELDS['testAll'])
        FIELDS['testPass'] = self.success_counter + int(FIELDS['testPass'])
        FIELDS['testFail'] = self.failure_count + int(FIELDS['testFail'])
        FIELDS['testError'] = self.error_count + int(FIELDS['testError'])
        FIELDS['testSkip'] = self.skipped + int(FIELDS['testSkip'])
        ...
        start_time = int(time.mktime(time.strptime(FIELDS['beginTime'], '%Y-%m-%d %H:%M:%S')))
        FIELDS['totalTime'] = str(end_time - start_time) + 's'  # 结束时间-开始是时间
        self.FIELDS = FIELDS
        # self.writeItem(str(self.FIELDS))
        return FIELDS

    def writeItem(self, result):
        ...
        for i in range(1, 100):
            overrideFill = override_path + "aa_"+str(i) + ".txt"
            if os.path.exists(overrideFill) is False:
                break
        with open(override_path + "aa.txt", 'rb') as file:
            body = file.readlines()
        with open(overrideFill, 'wb') as write_file:
            for item in body:
                if item.strip().startswith(b'var resultData'):
                    item = json.dumps(result, ensure_ascii=False, indent=4)
                    item = item.encode()
                    item = bytes(item) + b';\n'
...
class BeautifulReport(ReportTestResult, PATH):
    ...
    def report(self, description, filename: str = None, log_path='.', report_path="."):
        """
            生成测试报告,并放在当前运行路径下
        :param report_path: 生成report的文件存储路径
        :param log_path: log日志路径
        ...
        self.log_path = os.path.join(os.path.abspath(log_path), "log") if log_path == "." else log_path
        self.result_path = os.path.join(os.path.abspath(report_path), "testreport") if report_path == "." else report_path
        self.suites.run(result=self)
        self.stopTestRun(self.title)
        self.output_report()
        text = '\n测试已全部完成, 可前往【{}】查询测试报告。'.format(self.result_path)
    ...
    def output_report(self):
        ...
        override_path = os.path.abspath(self.result_path) if \
            os.path.abspath(self.result_path).endswith('/') else \
